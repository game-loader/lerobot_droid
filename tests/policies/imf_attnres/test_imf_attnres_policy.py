...
"""Regression tests for the IMF-AttnRes policy and upstream training contract."""
...
def make_libero_like_batch(
    *,
    ...
    batch.update({key: torch.rand(batch_size, 2, 3, IMAGE_SIZE, IMAGE_SIZE) for key in image_keys})
    return batch
...
def test_imf_attnres_sample_tr_uses_logit_normal_random_data_proportion_mask(monkeypatch):
    """Sample t/r from Logit-Normal and force r=t according to a per-sample random mask."""
    ...
    random_draws = torch.tensor([0.75, 0.25, 0.75, 0.25], dtype=torch.float32)
    rand_calls = []
    ...
    def fake_rand(batch_size, *, device=None, dtype=None):
        assert batch_size == 4
        rand_calls.append(batch_size)
        return random_draws.to(device=device, dtype=dtype)
    ...
    fm_mask = random_draws < config.data_proportion
    expected_r_before_sort = torch.where(fm_mask, raw_t, raw_r)
    ...
    torch.testing.assert_close(t[fm_mask], r[fm_mask])
    assert not torch.equal(fm_mask, torch.tensor([True, True, False, False]))
...
def test_imf_attnres_pseudo_huber_loss_matches_formula():
    ...
    expected = config.pseudo_huber_delta**2 * (torch.sqrt(1 + (error / config.pseudo_huber_delta) ** 2) - 1)
    ...
    def fake_fn(z, r_value, t_value, cond=None):
        ...
        if torch.equal(r_value, r) and torch.equal(t_value, s):
            assert not z.requires_grad, (
                "teacher-composed midpoint must be detached before second teacher call"
            )
...
def test_flat_conditioning_preserves_but_freezes_layerwise_checkpoint_keys():
    ...
    assert dormant
    assert not any(parameter.requires_grad for parameter in dormant.values())
    restored = get_policy_class(POLICY_NAME)(make_tiny_imf_attnres_config())
    restored.load_state_dict(policy.state_dict(), strict=True)


def test_parent_policy_load_initializes_missing_legacy_semigroup_teacher():
    config = make_tiny_imf_attnres_config()
    config.enable_semigroup_consistency = True
    ...
    restored = get_policy_class(POLICY_NAME)(config)
    restored.load_state_dict(source, strict=True)
    for key, value in restored.model.head.state_dict().items():
        torch.testing.assert_close(restored.model.semigroup_teacher_head.state_dict()[key], value)
    source.pop("model.head.head.weight")
    with pytest.raises(RuntimeError, match="Missing key"):
        restored.load_state_dict(source, strict=True)


def test_dct_padding_retains_legacy_frequency_index_mask_with_warning(monkeypatch):
    config = make_tiny_imf_attnres_config()
    config.do_mask_loss_for_padding = True
    config.dct_loss_high_freq_weight = 0.0
    model = IMFAttnResModel(config)
    batch = make_libero_like_batch()
    batch[OBS_IMAGES] = torch.stack([batch.pop(key) for key in IMAGE_KEYS], dim=2)
    batch["action_is_pad"] = torch.tensor([[False, False, True, True]]).expand(2, -1)
    losses = torch.arange(4.0).view(1, 4, 1).expand(2, 4, ACTION_DIM)
    monkeypatch.setattr(model, "_velocity_loss_from_error", lambda _: losses)
    with pytest.warns(UserWarning, match="frequency bins"):
        loss, _ = model.compute_loss(batch)
...
def test_imf_attnres_generate_actions_decodes_dct_latent_before_slicing(monkeypatch):
    ...
    expected = original_decode(latent)[
        :, config.n_obs_steps - 1 : config.n_obs_steps - 1 + config.n_action_steps
    ]
...
def _make_train_metrics() -> MetricsTracker:
    return MetricsTracker(
        ...
        metrics={
            ...
            "gpu_mem_gb": AverageMeter("gpu_mem_gb", ":.3f"),
        },
    )


def test_imf_attnres_update_policy_logs_model_diagnostics_and_gradient_norm():
    """Model diagnostics use upstream aggregation and gradient clipping."""
    policy_cls = get_policy_class(POLICY_NAME)
    config = make_tiny_imf_attnres_config()
    config.enable_imf_diagnostics = True
    policy = policy_cls(config)
    ...
    metrics, output_dict = update_policy(
        _make_train_metrics(),
        policy,
    ...
    assert isinstance(output_dict, dict)
    assert "imf_diagnostics/all/target_norm_mean" in output_dict
    assert np.isfinite(metrics.grad_norm.val)
    assert metrics.grad_norm.val >= 0.0
...
def test_imf_attnres_libero_style_train_pipeline_smoke(tmp_path):
    ...
    with (
        patch.object(datasets.config, "HF_DATASETS_CACHE", hf_datasets_cache),
        patch.object(datasets.config, "DOWNLOADED_DATASETS_PATH", hf_downloaded_cache),
        patch.object(datasets.config, "EXTRACTED_DATASETS_PATH", hf_extracted_cache),
        patch("lerobot.datasets.dataset_metadata.snapshot_download") as metadata_download,
        patch("lerobot.datasets.lerobot_dataset.snapshot_download") as data_download,
    ):
    ...
    dataloader = torch.utils.data.DataLoader(
        dataset, batch_size=2, shuffle=False, num_workers=0, drop_last=True
    )
