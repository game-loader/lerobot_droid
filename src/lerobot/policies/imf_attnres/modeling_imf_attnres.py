...
class IMFAttnResSmolVLMVLEncoder(nn.Module):
    ...
    def _preprocess_images(self, images: Tensor) -> Tensor:
        ...
        if self.vlm_resize_shape is not None:
            ...
            width, height = self.vlm_resize_shape
            flat_images = resize_with_pad(flat_images, height, width, pad_value=0)
...
class IMFAttnResPolicy(PreTrainedPolicy):
    ...
    def load_state_dict(self, state_dict, strict: bool = True, assign: bool = False):
        # Parent-policy loading does not invoke the child model's load_state_dict
        # override. Seed missing legacy teacher weights from the loaded online head.
        teacher_prefix = "model.semigroup_teacher_head."
        has_teacher = any(key.startswith(teacher_prefix) for key in state_dict)
        if has_teacher and self.model.semigroup_teacher_head is None:
            self.model._reset_semigroup_teacher()
        if self.config.enable_semigroup_consistency and not has_teacher:
            state_dict = copy(state_dict)
            for key in self.model.semigroup_teacher_head.state_dict():
                online_key = f"model.head.{key}"
                if online_key in state_dict:
                    state_dict[f"{teacher_prefix}{key}"] = state_dict[online_key]
...
class IMFAttnResModel(nn.Module):
    def __init__(self, config: IMFAttnResConfig):
        ...
        if getattr(config, "vlm_conditioning_mode", "flat_tokens") == "layerwise":
            ...
            # Keep serialized keys from the layerwise branch, but exclude dormant
            # parameters from flat-token optimization and DDP gradient tracking.
            for module in (self.head.layerwise_prefix_proj, self.head.layerwise_cross_attn):
                if module is not None:
                    module.requires_grad_(False)
        self._warned_dct_padding = False
    ...
    def _jvp_math_sdp_context(z_t: Tensor):
        # CPU fused SDPA also lacks the higher-order derivatives required by JVP.
    ...
    def compute_loss(
        ...
        if self.config.do_mask_loss_for_padding and "action_is_pad" in batch:
            if (
                self.config.action_latent_mode == "dct"
                and not self._warned_dct_padding
                and batch["action_is_pad"].any()
            ):
                warnings.warn(
                    "IMF DCT loss preserves legacy index masking: action_is_pad masks frequency bins, "
                    "not time-domain padding. Use complete action windows or action_latent_mode='identity' "
                    "when exact temporal masking is required.",
                    stacklevel=2,
                )
                self._warned_dct_padding = True
