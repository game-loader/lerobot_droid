...
"""LeRobot-native point-cloud and dual-wrist RGB Diffusion Policy.

The point encoder follows the RL-100/DP3 structure: shared per-point MLP,
global max pooling, and a projection layer. Its feature is concatenated with
two randomly initialized ResNet18 wrist features and encoded robot state. The
action denoiser and checkpoint contract reuse LeRobot Diffusion. The intended
deployment source is the real Franka Duo recorder's ZED depth sidecar plus
...
class DP3ObservationEncoder(nn.Module):
    ...
    def forward(self, state: Tensor, point_cloud: Tensor, wrist_images: Tensor) -> Tensor:
        ...
        if state.shape[0] != point_cloud.shape[0] or state.shape[0] != wrist_images.shape[0]:
            raise ValueError("DP3 state, point-cloud, and wrist RGB batch dimensions must match.")

        features = [self.point_net(point_cloud)]
        features.extend(encoder(wrist_images[:, index]) for index, encoder in enumerate(self.rgb_encoders))
...
class DP3DiffusionModel(DiffusionModel):
    ...
    def _prepare_global_conditioning(self, batch: dict[str, Tensor]) -> Tensor:
        ...
        if missing and OBS_IMAGES not in batch:
...
class DP3Policy(DiffusionPolicy):
    ...
    def select_action(self, batch: dict[str, Tensor], noise: Tensor | None = None) -> Tensor:
        """Select one action from point cloud, dual-wrist RGB, and robot state."""
