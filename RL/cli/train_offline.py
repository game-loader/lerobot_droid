...
def run(args: argparse.Namespace) -> Path:
    ...
    # RL-100 3D freezes the observation encoder for the whole offline phase so
    # the critic/dynamics/actor all reason over one stable representation.
    current.freeze_observation_encoder()
    ...
    # The actor optimizer owns only the trainable module parameters; the frozen
    # observation encoder stays out of every optimizer, so it never moves during
    # the offline phase.
    actor_optimizer = torch.optim.Adam(
        [parameter for parameter in current.policy.parameters() if parameter.requires_grad],
        lr=args.actor_lr,
    )
