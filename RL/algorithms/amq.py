...
class AMQEvaluator:
    ...
    def __init__(
        ...
        *,
        ...
        if not isinstance(dynamics, (StateDynamicsEnsemble, DP3FeatureDynamicsEnsemble)):
            raise ValueError("dynamics must be a StateDynamicsEnsemble or DP3FeatureDynamicsEnsemble")
        ...
        for name, policy in (
            ...
            if state_feature is None or (
                dynamics_state_dim is not None and tuple(state_feature.shape) != (dynamics_state_dim,)
            ):
                raise ValueError(f"{name} state feature shape is incompatible with dynamics")
            if action_feature is None or tuple(action_feature.shape) != (dynamics.action_packer.action_dim,):
                raise ValueError(f"{name} action feature shape is incompatible with dynamics")
    ...
    def _normalized_initial(self, observation: ObservationBatch, *, normalized: bool) -> ObservationBatch:
        state = _state_observation(observation, state_key=self.state_key, preserve_modalities=True)
        if normalized:
            return state.to(self.device)
        return self.behavior_policy.checkpoint.normalize_observation(state, convert_visual_uint8=True).to(
            self.device
        )
    ...
    def evaluate(
        ...
        *,
        ...
        action_valid = torch.ones(batch_size, chunk_size, dtype=torch.bool, device=self.device)
        ...
        for _step in range(self.rollout_horizon):
            ...
            if self.latent_mode:
                next_state_ensemble = self.dynamics.next_feature_history(latent_history, prediction)
            else:
                next_state_ensemble = self.dynamics.next_state_history(state_history, prediction)
            disagreement = next_state_ensemble.var(dim=0, unbiased=False).mean(dim=(-1, -2))
            max_disagreement = torch.maximum(max_disagreement, torch.where(active, disagreement, 0.0))
            ...
            if self.latent_mode:
                ...
                next_features[self.state_key] = torch.where(active_rows, predicted_next_state, current_state)
        ...
        result = AMQResult(
            ...
            mean_predicted_reward=float((reward_sum / active_count.clamp_min(1.0)).item()),
            mean_predicted_done=float((done_sum / active_count.clamp_min(1.0)).item()),
    ...
    def paired(
        ...
        *,
        ...
        candidate = self.evaluate(self.candidate_policy, observation, normalized=normalized, seed=seed)
        behavior = self.evaluate(self.behavior_policy, observation, normalized=normalized, seed=seed)
