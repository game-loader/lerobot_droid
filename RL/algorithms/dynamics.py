...
class DP3FeatureDynamicsEnsemble(nn.Module):
    ...
    def encode_observation(self, observation: ObservationBatch) -> Tensor:
        ...
        if features.ndim != 2 or features.shape[-1] != self.feature_encoder.output_dim:
            raise ValueError(
                "feature encoder must emit [batch, output_dim], got "
    ...
    def _select_model_features(self, features: Tensor) -> Tensor:
        if features.ndim == 3:
            ...
            return (
                ...
                else features.reshape(features.shape[0], -1)
            )
        if features.ndim != 2:
    ...
    @torch.no_grad()
    def predict(self, observation: ObservationBatch, action: Tensor, action_valid: Tensor) -> LatentDynamicsPrediction:
    ...
    def _targets(self, batch: DecisionBatch) -> tuple[Tensor, Tensor, Tensor, Tensor]:
        ...
        following_history = following.reshape(
            following.shape[0], self.n_obs_steps, self.latent_dim
        )
    ...
    def validation_loss(self, batch: DecisionBatch) -> float:
        model_input, target, reward, done = self._targets(batch)
        values = [
            self._member_losses(member(model_input), target, reward, done)[0]
            for member in self.members
        ]
    ...
    @torch.no_grad()
    def bootstrap_indices(self, batch_size: int, *, member_index: int, device: torch.device | str = "cpu") -> Tensor:
        _positive_int("batch_size", batch_size)
        if isinstance(member_index, bool) or not isinstance(member_index, int) or not 0 <= member_index < self.ensemble_size:
    ...
    def next_feature_history(
        ...
        if isinstance(features, Tensor) and features.ndim == 2 and features.shape[-1] == self.feature_encoder.output_dim:
        ...
        if prediction.feature_delta.shape != expected:
            raise ValueError(f"prediction feature_delta must have shape {expected}, got {tuple(prediction.feature_delta.shape)}")
...
class LatentDynamicsPrediction:
    ...
    def __post_init__(self) -> None:
        ...
        if not all(torch.isfinite(value).all().item() for value in (self.feature_delta, self.reward_logit, self.done_logit)):
