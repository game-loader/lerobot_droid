...
def _raw_feature_mapping(raw: Any, *, num_envs: int) -> dict[str, Tensor]:
    ...
    if isinstance(raw, Mapping):
        ...
        if not result:
            raise ValueError(
                f"observation mapping must contain one of {_STATE_ALIASES!r} or an image feature"
            )
    else:
...
def _final_observation_rows(raw: Any, *, num_envs: int) -> list[dict[str, Tensor] | None]:
    ...
    if isinstance(raw, Mapping):
        ...
        return [{key: value[index].clone() for key, value in mapped.items()} for index in range(num_envs)]
...
def _select_transition_frames(
    ...
    *,
    ...
    final_key = next((key for key in ("final_obs", "final_observation") if key in info), None)
    ...
    for world in np.flatnonzero(done):
        ...
        for key, base in next_frames.items():
            if key not in row:
                raise ValueError(f"done world {int(world)} final observation is missing feature {key!r}")
            value = row[key]
            ...
            selected[key][int(world)] = value.to(device=base.device, dtype=base.dtype)
    return selected
...
def _sequence_observation(items: Sequence[ObservationBatch]) -> ObservationBatch:
    ...
    return ObservationBatch({key: torch.stack([item.features[key] for item in items], dim=0) for key in keys})
...
class OnlineRolloutBatch:
    ...
    def __post_init__(self) -> None:
        ...
        if self.action.ndim != 4 or self.action.shape[0] != len(self.traces):
            raise ValueError(
                f"action must have shape [time,world,chunk,action], got {tuple(self.action.shape)}"
            )
        ...
        for name, value in (
            ...
            if value.shape not in {(time, worlds), (time, worlds, 1)}:
                raise ValueError(
                    f"{name} must have shape [time,world] or [time,world,1], got {tuple(value.shape)}"
                )
        ...
        if self.next_observation.features[next(iter(self.next_observation.features))].shape[:2] != (
            time,
            worlds,
        ):
...
class OnlineTrainer:
    ...
    def __init__(
        ...
        *,
        ...
        if not torch.equal(
            current_policy.checkpoint.active_action_mask, old_policy.checkpoint.active_action_mask
        ):
        ...
        if minibatch_size is not None and (
            isinstance(minibatch_size, bool) or not isinstance(minibatch_size, int) or minibatch_size <= 0
        ):
        ...
        if reward_mode not in {"terminal_success", "environment"}:
            raise ValueError(f"reward_mode must be 'terminal_success' or 'environment', got {reward_mode!r}")
    ...
    def set_runtime_contract(self, contract: Mapping[str, Any]) -> None:
        ...
        for key, value in contract.items():
            ...
            if (
                ...
                raise ValueError(f"online runtime contract value {key!r} must be a finite scalar")
    ...
    @staticmethod
    def _terminal_success(info: Mapping[str, Any], done: np.ndarray, reward: np.ndarray) -> np.ndarray:
                    ...
                    for key in ("is_success", "success"):
                        if key in item:
                            ...
                            if not isinstance(value, (bool, np.bool_)):
                                raise ValueError(f"final_info[{index}][{key}] must be bool, got {value!r}")
    ...
    def collect(
        ...
        *,
        ...
        for _decision in range(decisions):
            ...
            action_row = torch.zeros(
                (num_envs, chunk_size, action_dim), dtype=torch.float32, device=self.device
            )
            ...
            for action_index in range(chunk_size):
                action_np = (
                    raw_chunk[:, action_index, :].detach().to("cpu").numpy().astype(np.float32, copy=False)
                )
                ...
                reset_frames = _move_features(_raw_feature_mapping(next_raw, num_envs=num_envs), self.device)
                ...
                next_frames = _select_transition_frames(reset_frames, info, done_np, num_envs=num_envs)
                ...
                for world in range(num_envs):
                    if newly_done[world]:
                        ...
                        terminal_history[world] = {
                            key: terminal_appended[key][world].clone() for key in terminal_appended
                        }
                        final_success = bool(self._terminal_success(info, done_np, _env_reward)[world])
                ...
                for world in np.flatnonzero(newly_done):
                    ...
                    reset_history = _reset_history(
                        reset_features, self.current_policy.policy.config.n_obs_steps
                    )
            ...
            dones.append(
                torch.as_tensor(term_row | trunc_row, dtype=torch.bool, device=self.device).unsqueeze(-1)
            )
            ...
            terminated_rows.append(
                torch.as_tensor(term_row, dtype=torch.bool, device=self.device).unsqueeze(-1)
            )
            truncated_rows.append(
                torch.as_tensor(trunc_row, dtype=torch.bool, device=self.device).unsqueeze(-1)
            )
    ...
    def _advantages(self, rollout: OnlineRolloutBatch) -> tuple[Tensor, Tensor]:
        ...
        next_value = (
            rollout.next_values if rollout.next_values.ndim == 3 else rollout.next_values.unsqueeze(-1)
        )
    ...
    def update(self, rollout: OnlineRolloutBatch) -> dict[str, float]:
        ...
        actor_parameters = [
            parameter.detach().clone() for parameter in self.current_policy.policy.parameters()
        ]
        ...
        try:
            ...
            elif self.metrics_path is not None and not metrics_file_existed and self.metrics_path.exists():
    ...
    def _update_impl(self, rollout: OnlineRolloutBatch) -> dict[str, float]:
        ...
        permutation_generator = torch.Generator(device="cpu").manual_seed(
            self.seed + self.counters.global_updates
        )
        ...
        for _epoch in range(self.ppo_epochs):
            ...
            for start in range(0, batch_size, minibatch):
                ...
                losses: list[Tensor] = []
                try:
                    for step_index, new_log_prob in enumerate(
                        self.current_policy.iter_recomputed_log_prob(obs_mb, trace_mb)
                    ):
                        old_log_prob = self.current_policy.executable_log_prob(
                            trace_mb.old_log_prob[step_index : step_index + 1]
                        )
                ...
                if not all(
                    torch.isfinite(parameter).all().item() for parameter in self.value_network.parameters()
                ):
                    raise ValueError("value optimizer produced non-finite parameters")
                actor_losses.append(float(torch.stack(losses).mean().item()))
                self.counters = dataclasses.replace(
                    self.counters, actor_updates=self.counters.actor_updates + 1
                )
    ...
    def _flatten_traces(traces: Sequence[DenoisingTrace]) -> DenoisingTrace:
        ...
        first = traces[0]
        if any(
            trace.timesteps.shape != first.timesteps.shape
            or not torch.equal(trace.timesteps, first.timesteps)
