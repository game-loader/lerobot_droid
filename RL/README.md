...
Franka Duo policy (`34D` state, `20D` action, XYZ point cloud, and two wrist
RGB streams). The LeRobot policy, processor, dataset, and environment
contracts remain the source of truth.

**SmolVLA:** a separate single-task offline flow-RL entry point is available at
`python -m RL.cli.train_smolvla_offline`. One frozen SmolVLM supplies native actor
K/V and contextual tokens for compact query-Transformer Q/V heads. Attention
dynamics predicts next prefix embeddings and regenerates both representations
through that same frozen VLM for AM-Q. See
[`smolvla/README.md`](smolvla/README.md) for staged training, reward labels,
...
- State-only critics and dynamics use `observation.state`; DP3 instead uses a
  ...
  39D/14D, DP3 is 34D/20D; legacy action contracts require explicit conversion).
